      ...
      $.glob.sync(filePath, { cwd: cwd, root: '/' })
        .map(function (path) {
          if (config.get('minified')) {
            return originalOrMinified($.path.join(cwd, path));
          } else {
...
function originalOrMinified(file) {
    var isJS = file.substring(file.length - 3) === '.js';
    var isMinified = file.substring(file.length - 7) === '.min.js';
    if (isJS && !isMinified) {
      var minifiedFile = file.substring(0, file.length - '.js'.length).concat('.min.js');
      // check to see if there is a .min.js version in the same directory
      if ($.fs.existsSync(minifiedFile)) {
        return minifiedFile;
      } else {
        console.log('Couldn\'t find', minifiedFile);
        return file;
      }
    } else {
      return file;
